...
import {
  ...
  map,
  mapTo,
  mergeMap, mergeMapTo, pairwise,
  pluck, repeat, scan, switchMap, switchMapTo
} from "rxjs/operators";
...
export const transformationExamples = {
  buffer: {
    ...
    apply: function(inputs){
      return inputs[0].pipe(pluck('content')
        .buffer(inputs[1])
        .map(x => `[${x}]`));
    }
  },
  ...
  bufferToggle: {
    ...
    inputs: [
      [{t: 0, c: 1}, {t: 10, c: 2}, {t: 20, c: 3}, {t: 30, c: 4}, {t: 40, c: 5}, {t: 50, c: 6}, {t: 60, c: 7}, {
        t: 70,
        c: 8
      }, {t: 80, c: 9}],
      [{t: 15, c: 10}, {t: 45, c: 30}],
  ...
  concatMap: {
    ...
    apply: function(inputs, scheduler){
      return inputs[0].pipe(
        pluck('content'),
        concatMap(() => inputs[1].pipe(pluck('content')), (x, y) => '' + x + y));
    }
  },
  ...
  concatMapTo: {
    ...
    apply: function(inputs, scheduler){
      return inputs[0].pipe(
        pluck('content'),
        concatMapTo(inputs[1]).pipe(pluck('content')), (x, y) => '' + x + y);
    }
  },
  ...
  map: {
    ...
    apply: function(inputs){
      return inputs[0].pipe(map(evolve({content: (c) => c * 10})));
    }
  },
  ...
  mergeMap: {
    ...
    apply: function(inputs, scheduler){
      return inputs[0].pipe(
        pluck('content'),
        mergeMap(() => inputs[1].pipe(pluck('content')), (x, y) => '' + x + y, 2));
    }
  },
  ...
  mergeMapTo: {
    ...
    apply: function(inputs, scheduler){
      return inputs[0].pipe(
        pluck('content'),
        mergeMapTo(inputs[1].pipe(pluck('content'))), (x, y) => '' + x + y, 2);
    }
  },
  ...
  switchMap: {
    ...
    apply: function(inputs, scheduler){
      return inputs[0].pipe(
        pluck('content'),
        switchMap(() => inputs[1].pipe(pluck('content')), (x, y) => '' + x + y));
    }
  },
  ...
  switchMapTo: {
    ...
    apply: function(inputs, scheduler){
      return inputs[0].pipe(
        pluck('content'),
        switchMapTo(inputs[1].pipe(pluck('content')), (x, y) => '' + x + y));
    }
  },
};
